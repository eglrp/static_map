...
#define PRINT_ERROR(args)                                                      \
  (std::cout << "[FILE: " << BOLD << splited_file_name(__FILE__) << NONE_FORMAT       \
             << ", LINE: " << __LINE__ << "]" << RED << " [ERROR] " << NONE_FORMAT    \
             << (args) << std::endl)
#define PRINT_ERROR_FMT(fmt, ...)                                              \
  (printf("[FILE: %s, LINE: %d]" RED " [ERROR] " NONE_FORMAT fmt "\n",                \
          splited_file_name(__FILE__), __LINE__, __VA_ARGS__))

// DEBUG
#define PRINT_DEBUG(args)                                                      \
  (std::cout << "[FILE: " << splited_file_name(__FILE__)                       \
             << ", LINE: " << __LINE__ << "]" << BLUE << " [DEBUG] " << NONE_FORMAT   \
             << (args) << std::endl)
#define PRINT_DEBUG_FMT(fmt, ...)                                              \
  (printf("[FILE: %s, LINE: %d]" BLUE " [DEBUG] " NONE_FORMAT fmt "\n",               \
          splited_file_name(__FILE__), __LINE__, __VA_ARGS__))

// INFO
#define PRINT_INFO(args)                                                       \
  (std::cout << "[FILE: " << splited_file_name(__FILE__)                       \
             << ", LINE: " << __LINE__ << "]" << GREEN << " [INFO] " << NONE_FORMAT   \
             << (args) << std::endl)
#define PRINT_INFO_FMT(fmt, ...)                                               \
  (printf("[FILE: %s, LINE: %d]" GREEN " [INFO] " NONE_FORMAT fmt "\n",               \
          splited_file_name(__FILE__), __LINE__, __VA_ARGS__))
...
#define PRINT_WARNING_FMT(fmt, ...)                                            \
  (printf("[FILE: %s, LINE: %d]" YELLOW " [WARNING] " NONE_FORMAT fmt "\n",           \
          splited_file_name(__FILE__), __LINE__, __VA_ARGS__))
